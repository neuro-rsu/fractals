import { FractalElement, html, css } from '../../js/lit-fractal.js';

import '../button/button.js';
import '../icon/icons/icons.js';

customElements.define('property-grid', class PropertyGrid extends FractalElement {
            ...
            <div class="hheader">
                ...
                <div class="buttons" >
                    ${!this.showButtons ? html`` : html`
                        <fractal-button class="btn" ?toggled="${this.isShowFocused}" toggledClass="ontoggled" name="radio-button-checked" title="view focused" @click="${this._showFocused}"></fractal-button>
                        <fractal-button class="btn" name="refresh" title="refresh" @click="${(e) => this._expert(e)}"></fractal-button>
                        <fractal-button class="btn" name="${this.sort === 'none' ? 'hamburger' : 'sort'}" rotate="${this.sort === 'descending' ? 0 : 180}" title="sort" @click="${this._sort}"></fractal-button>
                        <fractal-button class="btn" ?toggled="${this.group}" toggledClass="ontoggled" name="list" title="group" @click="${this._group}"></fractal-button>
                        <fractal-button class="btn" ?toggled="${this.showFunction}" toggledClass="ontoggled" name="functions" title="showFunction" @click="${this._fn}"></fractal-button>
                        <fractal-button class="btn" ?toggled="${this.expertMode}" toggledClass="ontoggled" name="settings" title="expertMode" @click="${(e) => this._expert(e, true)}"></fractal-button>
                    `}
                </div>
            </div>
                ...
                ${Object.keys(this.item || {}).map(key => html`
                    <div class="group">
                       ${this.group ? html`<div class="group-header">${key} [${this.item[key].length}]</div>` : html``}
                        <lit-property-tree class="tree" .item="${this.item[key]}" .args="${this.args}"></lit-property-tree>
                    </div>
                `)}
            </div>
...
customElements.define('lit-property-tree', class LitPropertyTree extends FractalElement {
    static get properties() {
        return {
            expertMode: { type: Boolean, default: false, local: true },
            item: { type: Object, default: undefined },
            ...
            ${this._items.map((i, idx) => html`
                <div class="row ${this.$$ && this.$$.selection && this.$$.selection.includes(i) ? 'selected' : ''} ${this.focused === i ? 'focused' : ''}" style="border-bottom: .5px solid lightgray" @click="${(e) => this._focus(e, i)}">
                    <div style="display:flex;align-items:center;${'border-bottom: 1px solid ' + this.colorBorder}">
                        ${i.items?.length || i.is
                ? html` <fractal-button back="transparent" name="chevron-right" border="0" toggledClass="right90" .toggled="${i.expanded}"
                                    @click="${(e) => this._expanded(e, i)}" size="${this.iconSize - 2}"></fractal-button>`
                : html` <div style="min-width:${this.iconSize}px;width:${this.iconSize}px;min-height:${this.iconSize}px;height:${this.iconSize}px"></div>`}
                            ...
                            ${i.list && !i.readOnly
                ? html` <fractal-button back="transparent" name="chevron-right" border="0" rotate="90" @click="${(e) => this._openDropdown(e, i, idx)}"></fractal-button>` : html``}
                    </div>
                </div>
                <div class="complex">
                    ${(i.el || i.items.length) && i.expanded ? html`<lit-property-tree .item="${i.data}" .args="${this.args}" .level="${this.level + 1}"></lit-property-tree>` : html``}
                </div>
            `)}
        `
    }
    ...
    async _openDropdown(e, i, idx) {
        try {
            let val = await FRACTAL.show('dropdown', 'tester-cell', { type: i.type, value: i.value, props: { list: i.list } }, { parent: this.$refs('inp-' + idx), useParent: true, align: 'down', useParentWidth: true, addWidth: e.target.offsetWidth + 3 });
